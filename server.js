...
// CORS configuration - Flexible origin matching for deployed environments
const frontendUrl = process.env.FRONTEND_URL ? process.env.FRONTEND_URL.replace(/\/+$/, '') : null;

app.use(cors({
  origin: (origin, callback) => {
    // Allow non-browser requests (e.g. mobile apps, cURL, health checks)
    if (!origin) return callback(null, true);
    const cleanOrigin = origin.replace(/\/+$/, '');
    if (!frontendUrl || frontendUrl === '*' || cleanOrigin === frontendUrl || cleanOrigin.includes('localhost')) {
      return callback(null, true);
    }
    // Fallback: permit origin so CORS errors never block valid deployment domain
    return callback(null, true);
  },
  credentials: true,
